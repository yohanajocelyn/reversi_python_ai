...
class GameUI:
    ...
    def draw_intro_screen(screen, start_btn_rect):
        ...
        title_rect = title_text.get_rect(center=(GameLogic.WIDTH // 2, GameLogic.HEIGHT // 5))
        screen.blit(title_text, title_rect)

        # Subtitle text
        subtitle_text = subtitle_font.render("Human vas AI", True, text_color)
        subtitle_rect = subtitle_text.get_rect(center=(GameLogic.WIDTH // 2, title_rect.bottom + 30))
        ...
        for i, rule in enumerate(rules):
            ...
            rule_rect = rule_text.get_rect(center=(GameLogic.WIDTH // 2, start_y + i * 35))
        ...
        btn_text = btn_font.render("START", True, GameLogic.WHITE)
    ...
    def draw_game_over_screen(screen, game, reset_btn_rect):
        ...
        for r in range(GameLogic.ROWS):
            for c in range(GameLogic.COLS):
                if game.board[r][c] == GameLogic.BLACK_PIECE:
                    human_score += 1
                elif game.board[r][c] == GameLogic.WHITE_PIECE:
                    ai_score += 1
        ...
        ai_score_rect = ai_score_text.get_rect(center=(GameLogic.WIDTH // 2, title_rect.bottom + 60))
        screen.blit(ai_score_text, ai_score_rect)

        human_score_text = score_font.render(f"HUMAN (Black): {human_score}", True, GameLogic.WHITE)
        human_score_rect = human_score_text.get_rect(center=(GameLogic.WIDTH // 2, ai_score_rect.bottom + 30))
        screen.blit(human_score_text, human_score_rect)

        # Winner
        winner_text = winner_font.render(winner_text, True, text_color)
        winner_rect = winner_text.get_rect(center=(GameLogic.WIDTH // 2, human_score_rect.bottom + 60))
        ...
        btn_font = pygame.font.SysFont(None, 40)
        btn_text = btn_font.render("PLAY AGAIN", True, GameLogic.WHITE)
    ...
    def run_game(self):
        ...
        HUMAN_PLAYER = GameLogic.BLACK_PIECE
        AI_PLAYER = GameLogic.WHITE_PIECE
        ...
        start_btn_rect = pygame.Rect(
            (GameLogic.WIDTH // 2) - (btn_width // 2), 
            GameLogic.HEIGHT - 150, 
            btn_width, 
            btn_height
        )
